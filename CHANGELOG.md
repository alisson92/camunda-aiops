...
- `docs/persistencia-dados-agente.md` — análise de persistência do estado do agente (KnowledgeBase, runbooks, dedup cache); mapeamento do que se perde no restart e seu impacto; três opções de storage (PVC, SQLite, S3/MinIO) com trade-offs; três estratégias de retenção (CronJob, TTL no código, S3 Lifecycle); estimativa de volume de dados; recomendação por horizonte de tempo
